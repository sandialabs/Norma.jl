...
if __name__ == "__main__":
    ...
    sidesets = ["nsx--x", "nsx+-x", "nsy--y", "nsy+-y", "ssz-","nsz+-z"]
    ...
    #   Note: I don't construct a cAHGB ROM in this example since I know there is no forcing vector
    ...
    # Flip signs to match convention of K, H and G on the LHS
    K = -opinf_model.A_.entries
    B = opinf_model.B_.entries
    H = -opinf_model.H_.expand_entries(opinf_model.H_.entries)
    G = -opinf_model.G_.expand_entries(opinf_model.G_.entries)
    ...
    np.savez('cubic-opinf-operator',**vals_to_save)
