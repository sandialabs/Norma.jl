...
function plot_solns_overlap()
	...
	for i in 0:num_files-1
		...
		t2 = readdlm(@sprintf("02-time-%04d.csv", i), ',')
		...
		# Displacement
		d_ex = 0.5 .* a_ .* (
			exp.(-((zz .- c .* t1 .- b_) .^ 2) ./ (2s^2)) .+
			exp.(-((zz .+ c .* t1 .- b_) .^ 2) ./ (2s^2))
		) .- 0.5 .* a_ .* (
			exp.(-((zz .- c .* (T .- t1) .- b_) .^ 2) ./ (2s^2)) .+
			exp.(-((zz .+ c .* (T .- t1) .- b_) .^ 2) ./ (2s^2))
		)
		...
		v_ex =
			(c * 0.5 * a_ / s^2) .* (
				((zz .- c .* t1 .- b_) .* exp.(-((zz .- c .* t1 .- b_) .^ 2) / (2s^2)) .-
				 (zz .+ c .* t1 .- b_) .* exp.(-((zz .+ c .* t1 .- b_) .^ 2) / (2s^2)))
			) .+ (c * 0.5 * a_ / s^2) .* (
				((zz .- c .* (T .- t1) .- b_) .* exp.(-((zz .- c .* (T .- t1) .- b_) .^ 2) / (2s^2)) .-
				 (zz .+ c .* (T .- t1) .- b_) .* exp.(-((zz .+ c .* (T .- t1) .- b_) .^ 2) / (2s^2)))
			)
		...
		a_ex =
			0.5 .* a_ .* (
				-c^2 / s^2 .* exp.(-0.5 .* ((zz .- c .* t1 .- b_) .^ 2) ./ (s^2)) .+
				c^2 / s^4 .* ((zz .- c .* t1 .- b_) .^ 2) .* exp.(-0.5 .* ((zz .- c .* t1 .- b_) .^ 2) ./ (s^2)) .-
				c^2 / s^2 .* exp.(-0.5 .* ((zz .+ c .* t1 .- b_) .^ 2) ./ (s^2)) .+
				c^2 / s^4 .* ((zz .+ c .* t1 .- b_) .^ 2) .* exp.(-0.5 .* ((zz .+ c .* t1 .- b_) .^ 2) ./ (s^2))
			) .-
			0.5 .* a_ .* (
				-c^2 / s^2 .* exp.(-0.5 .* ((zz .- c .* (T .- t1) .- b_) .^ 2) ./ (s^2)) .+
				c^2 / s^4 .* ((zz .- c .* (T .- t1) .- b_) .^ 2) .* exp.(-0.5 .* ((zz .- c .* (T .- t1) .- b_) .^ 2) ./ (s^2)) .-
				c^2 / s^2 .* exp.(-0.5 .* ((zz .+ c .* (T .- t1) .- b_) .^ 2) ./ (s^2)) .+
				c^2 / s^4 .* ((zz .+ c .* (T .- t1) .- b_) .^ 2) .* exp.(-0.5 .* ((zz .+ c .* (T .- t1) .- b_) .^ 2) ./ (s^2))
			)
		...
		xlabel("z")
		ylabel("z-disp")
		title("displacement snapshot $(i+1) at time = $t1")
		...
		xlabel("z")
		ylabel("z-velo")
		title("velocity snapshot $(i+1) at time = $t1")
		...
		xlabel("z")
		ylabel("z-acce")
		title("acceleration snapshot $(i+1) at time = $t1")
		axis([minimum(z), maximum(z), -2.5e9 * scale, 2.5e9 * scale])
        legend(loc="lower right")

        PyPlot.pause(0.1)  # short pause to visualize updates
		...
		if save_figs == 1
			...
			savefig(filename)
			# .fig is a native Matlab format not directly supported by PyPlot,
			# so we skip or use something like .pdf if needed.
		end
