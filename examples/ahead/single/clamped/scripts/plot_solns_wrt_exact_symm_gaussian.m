...
for i=0:100:10000
  ...
  s = 0.02;
  T = 1e-3;
  d_ex = 1/2*a*(exp(-(z-c*t-b).^2/2/s^2) + exp(-(z+c*t-b).^2/2/s^2))...
         - 1/2*a*(exp(-(z-c*(T-t)-b).^2/2/s^2) + exp(-(z+c*(T-t)-b).^2/2/s^2));
  v_ex = c/2*a/s^2*((z-c*t-b).*exp(-(z-c*t-b).^2/2/s^2) - (z+c*t-b).*exp(-(z+c*t-b).^2/2/s^2))...
          + c/2*a/s^2*((z-c*(T-t)-b).*exp(-(z-c*(T-t)-b).^2/2/s^2) - ...
          (z+c*(T-t)-b).*exp(-(z+c*(T-t)-b).^2/2/s^2));
  a_ex = 1/2*a*(-c^2/s^2*exp(-1/2*(z-c*t-b).^2/s^2)+1/s^4*c^2*((z-c*t-b).^2).*exp(-1/2*(z-c*t-b).^2/s^2)-c^2/s^2*exp(-1/2*(z+c*t-b).^2/s^2)+1/s^4*c^2*((z+c*t-b).^2).*exp(-1/2*(z+c*t-b).^2/s^2)) - ...
             1/2*a*(-c^2/s^2*exp(-1/2*(z-c*(T-t)-b).^2/s^2)+1/s^4*c^2*((z-c*(T-t)-b).^2).*exp(-1/2*(z-c*(T-t)-b).^2/s^2)-c^2/s^2*exp(-1/2*(z+c*(T-t)-b).^2/s^2)+1/s^4*c^2*((z+c*(T-t)-b).^2).*exp(-1/2*(z+c*(T-t)-b).^2/s^2));
