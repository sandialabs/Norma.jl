...
- Adjoint-paired non-overlapping impedance coupling (default): both sides share one mortar cross-mass matrix, one pair impedance (harmonic mean), and one Robin parameter, and exchange the dynamically consistent d'Alembert reaction, making interface energy exchange sign-definite (dissipative) on conforming and non-conforming meshes (`adjoint pairing: false` restores the legacy per-side transfer; see `doc/notes/schwarz-coupling`)
...
- Multi-time-step (subcycled) Schwarz with genuine piecewise-linear time interpolation of the partner state (per-substep trajectory snapshots anchored at the stop start), compatible with adjoint pairing (Gravouil–Combescure interpolated-continuity structure, sign-definite interface dissipation)
