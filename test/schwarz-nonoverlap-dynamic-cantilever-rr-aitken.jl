...
const cantilever_rr_example = "../examples/nonoverlap/dynamic-same-step/cantilever-rr"

# Run the Robin-Robin DD cantilever (Newmark) for a few steps with the requested
# relaxation method.
function run_cantilever_rr_aitken(relaxation; num_steps=5)
    for f in ["cantilever-multi.yaml", "cantilever-clamped.yaml", "cantilever-free.yaml",
              "cantilever-clamped.g", "cantilever-free.g"]
        cp("$cantilever_rr_example/$f", f; force=true)
    end
...
@testset "Schwarz Nonoverlap Dynamic Cantilever Robin-Robin Aitken" begin
    ...
    # Robin-Robin relaxes a single force RHS, so every method drives the same
    # fixed point to the same Schwarz tolerance: the solution must agree.
