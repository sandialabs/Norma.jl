...
@testset "Single Implicit Matrix Free Dynamic Solid Cube" begin
    cp("../examples/single/implicit-dynamic-solid/cube/cube-matrix-free.yaml", "cube-matrix-free.yaml"; force=true)
    cp("../examples/single/implicit-dynamic-solid/cube/cube.g", "cube.g"; force=true)
    simulation = Norma.run("cube-matrix-free.yaml")
    integrator = simulation.integrator
    model = simulation.model
    rm("cube-matrix-free.yaml")
    rm("cube.g")
    rm("cube.e")
    avg_disp = average_components(integrator.displacement)
    avg_stress = average_components(model.stress)
