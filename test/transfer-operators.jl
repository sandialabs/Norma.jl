...
@testset "Transfer Operators" begin
    ...
    src_side_set_id = 5
    dst_side_set_id = 6
    src_T = create_traction_force(src_mesh, src_side_set_id)
    Norma.norma_log(0, :info, "Source side set:         $(length(src_T)) nodes")
    dst_T_real = create_traction_force(dst_mesh, dst_side_set_id)
    ...
    src_U = create_displacement(src_mesh, src_side_set_id)
    Norma.norma_log(0, :info, "Source side set:         $(length(src_U)) nodes")
    dst_U_real = create_displacement(dst_mesh, dst_side_set_id)
    Norma.norma_log(0, :info, "Destination side set:    $(length(dst_U_real)) nodes")
    W = Norma.get_square_projection_matrix(dst_model, dst_side_set_id)
    L = Norma.get_rectangular_projection_matrix(src_model, src_side_set_id, dst_model, dst_side_set_id)
    dst_U = (W \ I) * L * src_U
