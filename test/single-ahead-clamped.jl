...
@testset "AHeaD Single Dynamic Clamped HEX8" begin
    ...
    for i in 1:n
        disp_z_exact[i] = 1/2*a*(exp(-(z[i]-c*t-b)^2/2/s^2) + exp(-(z[i]+c*t-b)^2/2/s^2)) - 1/2*a*(exp(-(z[i]-c*(T-t)-b)^2/2/s^2) + exp(-(z[i]+c*(T-t)-b)^2/2/s^2))
        velo_z_exact[i] = c/2*a/s^2*((z[i]-c*t-b)*exp(-(z[i]-c*t-b)^2/2/s^2) - (z[i]+c*t-b)*exp(-(z[i]+c*t-b)^2/2/s^2)) + c/2*a/s^2*((z[i]-c*(T-t)-b)*exp(-(z[i]-c*(T-t)-b)^2/2/s^2) - (z[i]+c*(T-t)-b)*exp(-(z[i]+c*(T-t)-b)^2/2/s^2))
        acce_z_exact[i] = 1/2*a*(-c^2/s^2*exp(-1/2*(z[i]-c*t-b)^2/s^2)+1/s^4*c^2*((z[i]-c*t-b)^2)*exp(-1/2*(z[i]-c*t-b)^2/s^2)-c^2/s^2*exp(-1/2*(z[i]+c*t-b)^2/s^2)+1/s^4*c^2*((z[i]+c*t-b)^2)*exp(-1/2*(z[i]+c*t-b)^2/s^2)) - 1/2*a*(-c^2/s^2*exp(-1/2*(z[i]-c*(T-t)-b)^2/s^2)+1/s^4*c^2*((z[i]-c*(T-t)-b)^2)*exp(-1/2*(z[i]-c*(T-t)-b)^2/s^2)-c^2/s^2*exp(-1/2*(z[i]+c*(T-t)-b)^2/s^2)+1/s^4*c^2*((z[i]+c*(T-t)-b)^2)*exp(-1/2*(z[i]+c*(T-t)-b)^2/s^2))
    end
    ...
    @test acce_z_relerr ≈ 0.11433498445981101 atol = 1e-12
