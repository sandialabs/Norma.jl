...
@testset "AHeaD Overlap Dynamic Clamped HEX8" begin
    ...
    for i in 1:n0
        disp0_z_exact[i] = 1/2*a*(exp(-(z0[i]-c*t-b)^2/2/s^2) + exp(-(z0[i]+c*t-b)^2/2/s^2)) - 1/2*a*(exp(-(z0[i]-c*(T-t)-b)^2/2/s^2) + exp(-(z0[i]+c*(T-t)-b)^2/2/s^2))
        velo0_z_exact[i] = c/2*a/s^2*((z0[i]-c*t-b)*exp(-(z0[i]-c*t-b)^2/2/s^2) - (z0[i]+c*t-b)*exp(-(z0[i]+c*t-b)^2/2/s^2)) + c/2*a/s^2*((z0[i]-c*(T-t)-b)*exp(-(z0[i]-c*(T-t)-b)^2/2/s^2) - (z0[i]+c*(T-t)-b)*exp(-(z0[i]+c*(T-t)-b)^2/2/s^2))
        acce0_z_exact[i] = 1/2*a*(-c^2/s^2*exp(-1/2*(z0[i]-c*t-b)^2/s^2)+1/s^4*c^2*((z0[i]-c*t-b)^2)*exp(-1/2*(z0[i]-c*t-b)^2/s^2)-c^2/s^2*exp(-1/2*(z0[i]+c*t-b)^2/s^2)+1/s^4*c^2*((z0[i]+c*t-b)^2)*exp(-1/2*(z0[i]+c*t-b)^2/s^2)) - 1/2*a*(-c^2/s^2*exp(-1/2*(z0[i]-c*(T-t)-b)^2/s^2)+1/s^4*c^2*((z0[i]-c*(T-t)-b)^2)*exp(-1/2*(z0[i]-c*(T-t)-b)^2/s^2)-c^2/s^2*exp(-1/2*(z0[i]+c*(T-t)-b)^2/s^2)+1/s^4*c^2*((z0[i]+c*(T-t)-b)^2)*exp(-1/2*(z0[i]+c*(T-t)-b)^2/s^2))
    end
    ...
    @test acce0_z_relerr ≈ 0.11433470809918797 atol = 1e-12
    ...
    for i in 1:n1
        disp1_z_exact[i] = 1/2*a*(exp(-(z1[i]-c*t-b)^2/2/s^2) + exp(-(z1[i]+c*t-b)^2/2/s^2)) - 1/2*a*(exp(-(z1[i]-c*(T-t)-b)^2/2/s^2) + exp(-(z1[i]+c*(T-t)-b)^2/2/s^2))
        velo1_z_exact[i] = c/2*a/s^2*((z1[i]-c*t-b)*exp(-(z1[i]-c*t-b)^2/2/s^2) - (z1[i]+c*t-b)*exp(-(z1[i]+c*t-b)^2/2/s^2)) + c/2*a/s^2*((z1[i]-c*(T-t)-b)*exp(-(z1[i]-c*(T-t)-b)^2/2/s^2) - (z1[i]+c*(T-t)-b)*exp(-(z1[i]+c*(T-t)-b)^2/2/s^2))
        acce1_z_exact[i] = 1/2*a*(-c^2/s^2*exp(-1/2*(z1[i]-c*t-b)^2/s^2)+1/s^4*c^2*((z1[i]-c*t-b)^2)*exp(-1/2*(z1[i]-c*t-b)^2/s^2)-c^2/s^2*exp(-1/2*(z1[i]+c*t-b)^2/s^2)+1/s^4*c^2*((z1[i]+c*t-b)^2)*exp(-1/2*(z1[i]+c*t-b)^2/s^2)) - 1/2*a*(-c^2/s^2*exp(-1/2*(z1[i]-c*(T-t)-b)^2/s^2)+1/s^4*c^2*((z1[i]-c*(T-t)-b)^2)*exp(-1/2*(z1[i]-c*(T-t)-b)^2/s^2)-c^2/s^2*exp(-1/2*(z1[i]+c*(T-t)-b)^2/s^2)+1/s^4*c^2*((z1[i]+c*(T-t)-b)^2)*exp(-1/2*(z1[i]+c*(T-t)-b)^2/s^2))
    end
    ...
    @test acce1_z_relerr ≈ 0.11433470809930901 atol = 1e-12
    ...
    @test sim.controller.schwarz_iters ≈ [2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2] atol = 0
