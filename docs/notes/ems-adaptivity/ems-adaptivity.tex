...
Acceptance criterion &
  ...
  Any: the energy gate of Section~\ref{sec:monotonicity}, a geometric
  floor, or both \\
...
exemption from the acceptance gate, and the length band of \omegah{}
...
gate compares cavity sums, and a proposal could lower the sum while
creating one element worse than the analysis code tolerates, so the
second condition of \eqref{eq:gate} may be stated, in place of or in
...
\eqref{eq:gate}. With a prescribed target the same holds for
...
\item \emph{Edge split and the size target.} Midpoint placement with
  ...
  Remark~\ref{rem:relax}, then the gate. With a prescribed target,
  ...
  through the same gate as the others. The acceptance case is the tube
...
\begin{enumerate}[itemsep=2pt]
\item \emph{The gate and the kernel.} The single functional of
...
\item \emph{Geometric floor versus energy gate.} An operation may lower
  the cavity energy while creating an element below the geometric floor
  the analysis requires; the floor is part of the gate for that reason,
...
cavity operators, and the single gate through which all of them pass.
