...
\item \emph{Scope restriction to four-node tetrahedra.} Inherited from
  ...
  documentation.
\item \emph{Anisotropic targets.} The metric-field rule holds the metric
  fixed during a solve, evaluated at the centroid of the original
  element, exactly as the size field does; the re-baselining of
  Section~\ref{sec:intrinsic} applies to it unchanged, with the volume
  floor applied to the three principal sizes uniformly so that the shape
  and orientation of the target are preserved.
