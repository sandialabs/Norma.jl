...
\date{August 2026, revised September 22, 2026}
...
at $0.15$ and $0.47$. The measurements then qualified the acceptance on
the energy: the size operations toward a prescribed target are accepted
on the edge length in the target, and the swaps on the cavity minimum of
the scaled Jacobian, measured in the metric space under a metric, with
the boundary edges of flat patches swapped as well; with these the loop
matches the staggered workflow of Norma smoothing and
\code{improve\_mesh} on the distorted cube and comes within $0.01$ of its
mean on a plate refined in four stages.
\end{abstract}
...
\item \emph{Boundary edge swap}: a boundary edge whose two boundary
  faces lie in one side set (or in none) and make an angle below
  \code{boundary swap angle} is replaced by the edge between the far
  nodes of the two faces, the faces by the two that contain the new edge,
  in the side set as well, and the chain of elements behind them by a
  triangulation of the resulting polygon. \emph{Face swap}: an interior
  face shared by two elements into the three elements around the edge
  between their apexes; off by default (Section~\ref{sec:staggered}).
\item \emph{Acceptance test} (\code{accept\_proposal}): the energy of
  ...
  was already below it and the new worst is no worse. Under
  \code{size criterion: length} the splits and collapses of edges outside
  the band are accepted on the length alone (Section~\ref{sec:length});
  under \code{shape criterion: scaled Jacobian} the swaps and the
  shape-driven collapses and splits must raise the cavity minimum of the
  scaled Jacobian (Section~\ref{sec:staggered}). In both the energy
  remains a validity check, and under a metric every shape measure is
...
\item \emph{Driver}: candidates above the desired density dilated by
  ...
  split in every pass (before the swaps with \code{size operations
  first}, the splits longest first in the target), and shape-driven
  collapses and splits only in a pass without accepted swaps unless
  \code{shape operations every pass}; no operation may create an edge or
  a face that exists or was created earlier in the pass; compaction after
  every operator; refused proposals remembered for the phase and retried
  only when their cavity changes; passes until none accepts or a cap; the adapted mesh written (as
  \code{<output name>-adapted-<k>.g}) and smoothed again until a topology
  phase accepts nothing or the outer iterations are exhausted, the outputs of
  the smoothing phases after the first written with the suffixes
  \code{-s\_0002}, \code{-s\_0003}, and so on, and their times continued from
  one file to the next, so that ParaView opens them as one sequence. The smoothing phases end on the energy stagnation exit of
...
\item \emph{Tests} 127 to 131, 133, and 134: the kernel on hand-built cavities against
  ...
  of that face, energy decrease accounted for); the length criterion on
  the tube toward a finer and a coarser target (133); and the options,
  the ranking, the quality in the metric space, the face swap, the
  boundary swap, the guard against created entities, and a topology phase
  on the cube under three combinations of the options (134).
\end{itemize}
...
\paragraph{The residual and the cost.} Two more measurements, made
on sirius after the runs on rigel. The loop applied to the mesh that the
staggered workflow produced improves it further, from a minimum of
$0.38$ and a mean of $0.703$ to $0.44$ and $0.707$, so the acceptance
test admits nothing harmful and the operators are at least as capable;
the residual between the two workflows from the distorted input is the
path each takes. Of the differences in path, the order of the operators
within a pass is the one that matters: \code{improve\_mesh} refines and
collapses to the size band before it swaps, and with
\code{size operations first: true} the loop reaches a mean of $0.698$
and a minimum of $0.41$ from $0.692$ and $0.39$, against $0.703$ and
$0.38$ for the staggered workflow. Ranking the cavities worst first
(which the scaled Jacobian criterion now does), more alternation with
fewer passes per phase, and the shape-driven operations in every pass
were each measured and gain nothing. On one machine the loop is
deterministic, so these differences are real, but between machines the
floating point path changes the minimum by about $0.03$ and the mean by
$0.005$, which is the size of what remains.

The cost was a separate matter. On the plate the loop spent ten of its
twelve hours at the last stage in the topology phases and most of the
rest in smoothing runs at twelve seconds per L-BFGS iteration on half a
million elements. Profiles on rigel found two implementation defects
rather than anything in the algorithm. The force evaluation of a
smoothing model allocated 19 KB per element and scaled by only a factor
of two on sixteen threads: the smoothing reference was built from column
slices and dynamic matrices, the size field was called through an
untyped function field, and the tangent by automatic differentiation
used a runtime-sized vector, which left its return type, and with it the
whole constitutive tuple even without a tangent, inferred as \code{Any},
so that everything downstream was dynamic. With a static reference, a
typed barrier for the size field, and a static input to the
differentiation, an evaluation of 85 thousand elements goes from $1.48$
s and $1.5$ GB to $0.11$ s and $44$ MB, and on 505 thousand elements
from $11.1$ to $0.9$ microseconds per element on sixteen threads. The
topology phase tried every candidate edge in every pass and computed the
energies of every configuration of a swap; nodes do not move within a
phase, so a refused proposal is now remembered for the phase and retried
only when an element around it has changed, the configurations of a
swap are scored by the scaled Jacobian with the energy of the chosen one
computed once as the validity check, and the candidates under that
criterion are the elements below \code{desired scaled Jacobian}. On the
converged stage 4 mesh the first swap pass goes from 197 to 92 s and the
following passes from about 200 to 7 s; the first three stages of the
plate, rerun, took 3, 7, and 39 minutes.

\paragraph{Quality in the metric space, and the order of the splits.}
Rerunning the anisotropic examples with the faster code reproduced their
energies step for step, but the loop under a metric target did not
converge: on the tube with the graded axial size of the anisotropic note
the operations per iteration stayed at 220 to 440 and the smoothed
energy rose from 795 to 3061. The shape measures were the scaled Jacobian
in physical space, which penalizes the elongation the target asks for, so
the swaps undid what the smoother did. Every shape measure of the loop,
the criterion, the floor, the candidates, and the ranking, is now the
scaled Jacobian of the element mapped by the metric factor $\bs F_M$
sampled on it, in which an element that matches the target is regular;
without a metric the measure is unchanged, since a scalar size scales all
edges alike. The tube then converges (167, 72, 49, 37, 20, 13 operations),
from 5657 to 6043 elements, with the metric quality from a minimum of
$0.17$ and a mean of $0.66$ to $0.43$ and $0.77$, $99.6$ percent of the
edges inside the band in the metric against $87.5$ percent on the input,
and every boundary node on its surface
(\code{examples/ems/tube/tube-metric-adaptive.yaml}). On the plate the
order of the operators gained nothing, refinement being what dominates
there, but the order of the splits did: they are tried longest first in
the target, as in longest-edge bisection, which is what bounds the shape
of the children: the four stages then reach means of $0.682$, $0.692$,
$0.701$, and $0.702$ with minima of $0.39$, $0.38$, $0.33$, and $0.32$,
from $0.671$, $0.680$, $0.693$, $0.696$ and $0.37$, $0.32$, $0.22$,
$0.33$ before, against $0.690$, $0.700$, $0.709$, $0.713$ and $0.40$,
$0.37$, $0.35$, $0.32$ for the staggered workflow, with 3389, 15043,
81616, and 486814 elements. The remaining $0.01$ in the mean is the
subdivision itself: \code{improve\_mesh} refines an element whose edges
are all long with a template, one into eight, whereas bisection one edge
at a time gives the same count with worse shapes. With the faster code
the four stages of the plate take 40 minutes on 16 threads, against about
14 hours before.
...
The measurements below record how the example came to its present
configuration; its current results, with the scaled Jacobian criterion,
boundary swaps, the size operations first, and the splits longest first,
are at the end of Section~\ref{sec:staggered}, with the comparison
against the staggered workflow and the present cost of 40 minutes for
the four stages.
...
elements. Under the length criterion, with a floor of $0.15$, equal moduli, and
the energy criterion for the swaps, the stages gave
\begin{center}
\begin{tabular}{lrrrrr}
\toprule
...
230 minutes on 16 threads before the performance work of
Section~\ref{sec:staggered}; the last is dominated by the eight smoothing
...
The risks this note ranked before the implementation, and how each was
resolved, followed by the questions that remain open.

\begin{enumerate}[itemsep=2pt]
\item \emph{The acceptance test and the kernel.} The kernel was
  validated on hand-built cavities, against an independent evaluation
  and the assembled energy, before the loop was built (test 127). The
  single functional then proved the wrong test for two classes of
  operation: the size operations toward a target, refused by the cavity
  sum beyond a ratio of about $1.5$ (Section~\ref{sec:length}), and the
  swaps, better judged by the cavity minimum of the scaled Jacobian
  (Section~\ref{sec:staggered}). The energy remains the objective of the
  smoothing and the validity check of every operation.
\item \emph{Boundary collapses on curved surfaces.} Resolved by
  construction: a collapse keeps the position of the surviving node,
  which must lie on every surface of the removed node, so no projection
  follows the test.
\item \emph{Four-node tetrahedra only.} A decision, stated in the input
  documentation; higher-order meshes are produced by conversion after
  the loop.
\item \emph{Energy thresholds across target sizes.} The thresholds are
  on the density and the cavity comparison on the sums
  (Section~\ref{sec:monotonicity}), as planned.
\item \emph{Geometric floor versus energy test.} The floor is part of the
  acceptance test under every criterion.
\end{enumerate}

Open questions:
\begin{enumerate}[itemsep=2pt]
\item \emph{Subdivision.} On the plate the loop remains about $0.01$
  below the staggered workflow in the mean scaled Jacobian. The
  difference is the subdivision: \code{improve\_mesh} refines an element
  whose edges are all long with a template, one into eight, whereas
  bisection one edge at a time gives the same count with worse shapes.
\item \emph{Termination under the scaled Jacobian criterion.} Descent of
  the energy (Proposition~\ref{prop:descent}) then covers the smoothing
  phases alone; the topology phases terminate on the strict rise of the
  cavity minimum, which bounds the number of operations but not the
  energy across a phase.
\item \emph{Over-constrained targets.} A target that asks for more
  elements across a direction bounded by fixed faces than the mesh has is
  a strict local minimum of the smoothing; the topology reaches it
  (\code{docs/notes/ems-anisotropic}), but the loop does not detect the
  situation, and a smoothing run alone reports convergence.
\item \emph{A metric from a solution.} The loop takes its target as
  given; the construction of a metric from the recovered Hessian of a
  field is the next step of the anisotropic note.
\end{enumerate}
...
it is in place as of September 22, 2026, with every shape measure taken
in the metric space when the target is a metric; the measurements of
