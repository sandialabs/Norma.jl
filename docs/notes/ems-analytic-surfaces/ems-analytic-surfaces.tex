...
\begin{description}[style=unboxed,leftmargin=0pt,itemsep=2pt]
  ...
  \item[Phase 4 (optional).] Edges and vertices ($m>1$) end-to-end for the
  analytic route, plus refinements: a symmetric smoothstep or harmonic weighting
  if a $C^1$ ramp is ever wanted, and caching of the compiled $g,\grad g$ per
  node set alongside the existing size-field closure.
\end{description}
