...
\date{September 2026, revised September 23, 2026}
...
remain on the domain boundary, sliding within it. Before this work the boundary
restriction was imposed with axis-aligned Dirichlet conditions on the faces of a
...
reduction to the current cube behavior. Because a Genesis/Exodus file carries no
analytic geometry---a curved boundary is stored only as a soup of flat element
facets, verified below---the note treats two interchangeable surface sources
feeding a common constraint: the user-supplied analytic level set (exact but
requires a formula) and the mesh's own boundary facets (general but only as
smooth as the input mesh).
\end{abstract}
...
(equal-volume, average-edge-length, \code{max}, a user size field, or, since
September 2026, a metric tensor field that also sets the shape and
orientation of the target; see \code{create\_smooth\_reference} and
\code{create\_metric\_reference} in \code{model.jl}), and a quasi-static solve
...
which level sets apply to which boundary side set; a node's feature dimension is
the number of level sets it accumulates across the side sets it belongs to
(one per smooth face, two along a shared edge, three at a corner).

\section{Surface representation and evaluation}
\label{sec:input}

The constraint model of Section~\ref{sec:model} needs only two quantities at
each constrained node: the signed surface residual $g(~x)$ and the unit normal
$~n=\grad g/\norm{\grad g}$ (one such pair per incident face at an edge or
vertex). Everything downstream is agnostic to \emph{how} they are produced.
There are two sources, and which applies is dictated by what the mesh file
actually carries.

\subsection{What a Genesis/Exodus file stores}
\label{sec:genesis}

A Genesis file records no analytic geometry. Cubit-generated primitives confirm
it: a curved boundary exported as a side set is stored as two integer arrays,
\code{elem\_ss} (element numbers) and \code{side\_ss} (local face index on each
element), with unit distribution factors---a list of (element, local-face)
pairs, that is, a soup of flat element facets. For a meshed sphere the
``spherical'' side set is just the outward hexahedral faces (\code{elem\_ss} the
24 surface elements, \code{side\_ss} all equal to the same local face); nothing
records that it is a sphere, its radius, or its normals. The curvature lives
\emph{only} in the nodal coordinates, and node sets are plain node lists. Two
consequences follow: the true smooth surface must be supplied by the user if it
is wanted, because the file cannot provide it; yet the mesh already carries a
usable---if piecewise-flat---representation of the boundary in the side-set
facets themselves.

\subsection{Analytic level sets}
\label{sec:analytic}
...
style, each entry binds a boundary \emph{side set} to one or more surfaces $g$;
the constraint applies to exactly the nodes of that side set, and the surface
count sets the feature dimension. A side set is the natural anchor because it is
already the boundary-face entity in the mesh: the same side set names the
constrained nodes for the analytic route \emph{and}, for the discrete route of
Section~\ref{sec:discrete}, supplies the facets---so one key serves both surface
sources.
\begin{verbatim}
model:
  type: mesh smoothing
  ...
  boundary surfaces:
    - side set: lateral          # face: 1 level set, 2 free DOF
      surfaces: ["x^2 + y^2 - R^2"]
    - side set: top_rim          # edge: 2 level sets, 1 free DOF
      surfaces: ["x^2 + y^2 - R^2", "z - H"]
    - side set: mount_corner     # vertex: 3 level sets, pinned
      surfaces: ["x", "y", "z - H"]
\end{verbatim}
Constants such as \code{R} and \code{H} are substituted into the string as they
already are for other expressions. A boundary side set with no listed surface is
left free (interior-like), and any Cartesian face can still be written as a
linear level set (e.g.\ \code{"x"}), so the cube case is a special case of the
schema. A node shared by two side sets (a common edge) accumulates both level
sets and is thereby constrained to their intersection, realizing the
$m=2$ hierarchy of Section~\ref{sec:model} directly from the side-set
membership.

\subsection{Discrete boundary facets}
\label{sec:discrete}

When no analytic form is available---an imported or otherwise formless
geometry---the boundary side set supplies the surface directly. Constrain each
boundary node to the union of its side-set facets in the reference
configuration by closest-point projection: the foot point is the drift target,
the facet normal is $~n$, and the signed distance to the facet is the residual
$g$ in \eqref{eq:normalconstraint}. Norma already has this machinery---
\code{closest\_point\_projection} and the containing-facet scan used by the
overlap-energy blending---so the discrete evaluator is largely an assembly of
existing parts and requires no new user input beyond naming the boundary side
set.

Faces, edges, and vertices are recovered by a feature-angle test on the facet
normals incident to a node: a single cluster of normals is a smooth face
($m=1$, project onto the boundary); two clusters mark an edge ($m=2$, constrain
to the ridge where the groups meet); three or more mark a corner ($m=3$,
pinned). This is the discrete analogue of the level-set count of
Section~\ref{sec:model}.

\emph{Trade-off.} The discrete surface is only as smooth as the input faceting:
nodes slide on the piecewise-flat boundary and cannot recover a surface smoother
than the mesh already resolves. For quality-driven smoothing---which
redistributes nodes without refining the boundary---that is usually the intent,
and it is the only option when the geometry has no closed form. The analytic
route is preferable when the smooth surface is known and sub-faceting fidelity
on the boundary matters. The two are combinable: analytic where a formula is
known (primitive faces), discrete elsewhere.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Discrete evaluator (formula-free).} Smooth the same sphere using
  only its boundary side-set facets (Section~\ref{sec:discrete})---no analytic
  surface---and confirm the result matches the analytic-sphere run to the
  faceting scale, and that the feature-angle test tags the smooth surface as a
  face (no spurious edges). This validates the general-geometry path on a
  geometry whose analytic form is, in this case, known for comparison.
\end{enumerate}
...
\begin{description}[style=unboxed,leftmargin=0pt,itemsep=2pt]
  \item[Phase 1 \emph{(implemented)}.] Level-set input
  (Section~\ref{sec:input}) plus the penalty enforcement
  (Section~\ref{sec:penalty}) and the cylinder test. A \code{Surface} boundary
  condition binds a side set to a symbolic $g$; its penalty force $\kappa\,g\,
  \grad g$ (Gauss--Newton tangent $\kappa\,\grad g\,\grad g^{\top}$) is assembled
  after \code{evaluate} exactly as the Robin BC is, with $\grad g$ from
  \code{Symbolics.gradient}, and a node on two side sets accumulates both
  penalties (the edge case) for free. Validated on a perturbed tet cylinder
  (\code{examples/\allowbreak ems/\allowbreak cylinder/}): every boundary node
  starts off its surface, and after smoothing the lateral residual
  $\max|x^2+y^2-1|$ drops from $1.2\times10^{-1}$ to $9\times10^{-4}$ at
  $\kappa=10^6$, scaling as $1/\kappa$ (the penalty signature), with no inverted
  elements and reduced element-volume spread. This derisked the input parsing,
  the \code{Symbolics} gradient path, and the membership/normal evaluation with
  the smallest possible code change.
  \item[Phase 2 \emph{(matrix-free implemented)}.] The exact local-frame
  constraint (Section~\ref{sec:exact}), selected by \code{enforcement: exact}
  (now the default; \code{enforcement: penalty} keeps Phase 1). For the
  matrix-free solvers the residual is projected onto each constrained node's
  tangent subspace (an orthonormal basis of the constraint gradients via a $3
  \times m$ QR) and, at the top of every \code{evaluate}, each node is
  closest-point-projected back onto its surface(s) by a normal-subspace Newton
  ($\delta~x = -A^{+}~g$). On the cylinder this holds the lateral residual at
  $\sim\!10^{-13}$ (return-to-surface tolerance), independent of any penalty
  parameter; the reduce-to-cube regression reproduces the Dirichlet smoothing to
  $\sim\!10^{-13}$ relative, confirming the axis-aligned special case. The Newton
  (\code{HessianMinimizer}) nodal rotation is not yet wired---that solver aborts
  on \code{enforcement: exact} with a message pointing to \code{penalty}---and is
  the remaining Phase~2 piece.
  \item[Phase 3.] The discrete-facet evaluator
  (Section~\ref{sec:discrete}) as the general-geometry surface source, sharing
  Phase~2's enforcement and adding the feature-angle detection of edges and
  vertices. This is the path for imported meshes with no closed form, and it is
  what the Genesis representation makes the natural default; validate on the
  sphere against its analytic counterpart.
  \item[Phase 4 \emph{(edges and vertices implemented)}.] Edges and vertices
  ($m>1$) end to end for the analytic route: a node in the side sets of two
  \code{Surface} conditions collects both constraints and slides along their
  intersection, a node in three is fixed at their common point, and gradients
  that are nearly linearly dependent at a node abort
  (Section~\ref{sec:robustness}). The reduce-to-cube regression exercises the
  edges and corners of six planes, and the tube examples
  (\code{examples/\allowbreak ems/\allowbreak tube/}) the circular edges
  where the cylinders meet the caps. What remains of this phase are the
  ...
  \item[Use by the adaptivity loop \emph{(implemented)}.] The topological
  operations of \code{docs/\allowbreak notes/\allowbreak ems-adaptivity} keep the boundary on the same
  surfaces: a node that a split adds on a boundary edge is returned to the
  surfaces of the boundary faces of the edge by the closest-point projection
  of Phase~2, under either enforcement; a collapse removes a boundary node
  only onto a node of the same surfaces, along a boundary edge; and a boundary
  swap acts only within one side set, so within one surface. The
  \code{Surface} conditions must therefore be given on side sets, which the
  operations keep up to date, rather than on node sets.
\end{description}

The two regressions are the test sets \code{surface\_\allowbreak constraint\_\allowbreak cylinder}
(penalty against exact on the perturbed cylinder) and
\code{surface\_\allowbreak constraint\_\allowbreak reduces\_\allowbreak to\_\allowbreak cube} of \code{test/smoothing.jl}
(test 51). The adaptivity tests 131 and 133 check that every boundary node
of a tube refined by splits lies on its cylinder to $10^{-8}$, and test 130
that a boundary node collapses only along a boundary edge onto a node of the
same surfaces.
