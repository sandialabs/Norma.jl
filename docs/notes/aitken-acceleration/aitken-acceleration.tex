...
Irons--Tuck variant (\texttt{relaxation: aitken recursive}) and the non-recursive secant
...
\section{Version 1: recursive Irons--Tuck Aitken (\texttt{relaxation: aitken recursive})}
...
Koliesnikova~\cite{Koliesnikova}, works with the differences of the
\emph{map outputs} $T(\bm{g}^{(n)})$ rather than of the iterates. Define
\begin{equation}
    \beta^{(n)} \;=\;
...
\paragraph{Notation.} Reference~\cite{Koliesnikova} writes the
fixed-point residual itself as $\delta g^{n}$, so the quantity
$\delta g^{n} - \delta g^{n-1}$ appearing there in the numerator and
denominator of~\eqref{eq:mapoutput} is this note's residual difference
$\bm{\delta}^{(n)}$ --- the shared letter $\delta$ denotes different
quantities in the two notations. Reference~\cite{Koliesnikova} also
indexes the sweep \emph{output} ($g^{n} = T(g^{n-1})$), whereas this
note evaluates $T$ and $\bm{r}$ at a single iterate level; the residual
definitions coincide after shifting the iterate indices by one.
...
\begin{tabular}{@{}lll@{}}
\toprule
Ref.~\cite{Koliesnikova} & This note & Meaning \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Correspondence between the notation of
Ref.~\cite{Koliesnikova} and that of this note. The mixed iterate
...
\texttt{aitken N0 parameter}, default $1$). Norma counts Schwarz sweeps from
$k = 0$. The two versions warm up differently
(\texttt{relaxation\_aitken\_recursive\_theta!} and
\texttt{relaxation\_aitken\_secant\_theta!} in \texttt{src/schwarz.jl}):

\begin{itemize}
\item \emph{Version 2 (secant)}: sweeps $k < N_0$ apply $\omega^{(1)}$
while recording the residual and the iterate; the first dynamic
factor~\eqref{eq:secant} is computed at $k = N_0$, once a previous
residual and a previous iterate are on record.
\item \emph{Version 1 (recursive)}: sweeps $k < N_0$ apply $\omega = 1$
(no relaxation, regardless of $\omega^{(1)}$) and keep the residual
history cleared; at $k = N_0$ no previous residual exists, so the factor
$\omega^{(1)}$ is applied and seeds the recursion; the first dynamic
factor~\eqref{eq:ironstuck} is computed at $k = N_0 + 1$, with
$\omega^{(N_0)} = \omega^{(1)}$ as the carried factor.
\end{itemize}

With the default $\omega^{(1)} = 1$ the warm-up factors of the two
versions coincide; for $\omega^{(1)} \ne 1$ Version~1 starts relaxing one
sweep later than Version~2.
...
\begin{equation}
    ...
    \label{eq:recover}
\end{equation}
The \texttt{relaxation: aitken recursive} variant instead relaxes
...
\texttt{aitken recursive} & $\bm{u},\bm{v},\bm{a}$ (same $\omega$) \\[1.4em]
...
\bibitem{Koliesnikova} D.~Koliesnikova, personal communication, July 2026.
\end{thebibliography}
