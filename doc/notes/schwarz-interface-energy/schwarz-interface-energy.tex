...
d'Alembert interface flux --- they are now implemented and validated:
implicit--implicit drift improves from $-9.6\%$ to $-0.03\%$ on conforming
and mildly nonconforming meshes, and every injection pathology disappears
(Sec.~\ref{sec:nonoverlap}). This note records the evidence, the theory, the
phased redesign, and the validated fix.
\end{abstract}
...
(i) the exchanged flux becomes the dynamically consistent d'Alembert reaction
...
\subsection{Explicit subdomains: not yet}

With an explicit (central-difference) subdomain on either side, the paired
coupling is violently unstable ($+10^3$ to $+10^5\,\%$): the consistent flux
couples the partner's \emph{acceleration} into the interface force, an
algebraic loop that the implicit pairs resolve within the Schwarz iteration
but that explicit time levels evaluate lagged. This is precisely the
Phase~4 prerequisite (implicit interface terms between explicit interiors,
Liu et al.'s IMEX-Newmark); the implementation warns loudly and the
...
(Sec.~\ref{sec:nonoverlap}): with the consistent d'Alembert flux and the
...
dissipation at strong contrast, with no injection anywhere. The practical
recommendation is therefore: conforming meshes within the overlap family, or
the adjoint-paired nonoverlap variant (\code{adjoint pairing: true}, implicit
subdomains) when the decomposition must be nonconforming and the energy
budget matters. Extending the pairing to explicit subdomains (IMEX interface
treatment) and the localized-Lagrange-multiplier frame construction
