...
node coincides with a fine node. CFL denotes the ratio of an explicit
subdomain's time step to its central-difference stability limit.
...
interface exchange introduces is never removed; with no implicit member in the
...
is thus a trade-off: it stabilizes explicit members (CFL margin) and
degrades implicit ones through interpolation dissipation.
...
EE, and the effective remedy remains CFL margin via subcycling
(Sec.~\ref{sec:tenms}).
...
\item \textbf{All-explicit interfaces}: provide CFL margin (e.g.\ subcycle
  ...
  the grid-frequency content the exchange introduces.
