...
implemented in Norma.jl and for the study of their energy behavior in dynamic
solid mechanics. It records, in order: the Robin and impedance transmission
conditions on shared (nonoverlap) interfaces and their reflection properties;
the Dirichlet and impedance conditions on overlap boundaries, including the
consistent partner traction that makes the impedance-overlap condition exact
on conforming meshes (energy error $59\% \to 0.08\%$ over $1$\,ms, with no
adjustable parameters); a systematic transfer-fidelity study on nonconforming
meshes showing that every increase in transfer fidelity worsens the interface
energy exchange, because the residual error is a disagreement between the two
grids' discrete dispersion relations rather than a transfer defect; a
parametric energy study ($151$ configurations) showing that the interface work
of the overlap coupling is sign-indefinite --- neighboring mesh ratios swing
from $-53\%$ to $+174\%$ to $-66\%$ drift --- and that at a ten-millisecond
horizon $74$ of $108$ nonconforming overlap runs diverge; the theoretical
requirements for energy consistency (a single-valued interface traction acting
through $L^2$-adjoint transfer operators, which makes the interface power
telescope to a sign-definite dissipation on the interface jump); and the
resulting redesign. The adjoint-paired nonoverlap impedance coupling ---
one shared cross-mass matrix, force transfers adjoint to the partner's
kinematic transfers, a harmonic-mean pair impedance, the dynamically
consistent d'Alembert interface reaction, and an implicit--explicit (IMEX)
treatment of the interface rows for explicit subdomains --- conserves energy
to $0.3\%$ over $1$\,ms in all four integrator combinations on conforming and
mildly nonconforming meshes, never injects energy at any mesh ratio, and holds
the monodomain trajectory over ten thousand steps. Subcycling with genuine
piecewise-linear time interpolation of partner data, the measured
incompatibility of exact interface-work telescoping with a partitioned
exchange, and the slot-keyed relaxation state required for controller stops
larger than a subdomain time step complete the picture. Practical
recommendations and open questions close the note.
\end{abstract}
...
partner data to a side's interface nodes, a subscript $p$ (in transmission
conditions written per side) the partner field evaluated at this side's
coupling boundary, and $\jump{\cdot}$ an interface jump.
...
energies counts the overlap region twice and pollutes the diagnostic: a
...
misprices the short-wavelength content of the sharp release front twice over
(the consistent and lumped Rayleigh quotients diverge at high wavenumber, and
the integer-step velocity aliases the staggered content): a monodomain
central-difference run of this benchmark reads $-8.2\%$ at $1$\,ms in the
consistent-mass integer-step metric and $\pm 0.000\%$ in
\eqref{eq:staggered}. Norma's energy output evaluates \eqref{eq:staggered}
for central-difference subdomains. Explicit-subdomain results below use this
metric unless noted.
...
Cholesky-factorized) nodal recovery is force-enabled for any model carrying
this condition; lumped recovery's $O(h)$ nodal-stress error feeds directly
into the transmission condition and dominates the residual energy error
($13.6\%$ vs.\ $1.5\%$ with consistent recovery; Sec.~\ref{sec:ladder}).
...
(Sec.~\ref{sec:redesign}) or by pointwise interpolation as a legacy option.
At the Schwarz fixed point all three jumps in \eqref{eq:imp-overlap} vanish
for the monodomain solution, so on conforming meshes the converged coupled
solution is the monodomain solution and the condition needs no parameter
tuning: $~Z$ is fixed by the partner material and $\alpha$ may be zero. A
step-dependent multiplier on $~Z$ (\code{impedance\_scale}) rescales the
whole operator; with the traction term in place the conserving scale is the
physical one (scale $= 1$), and the historical role of the scale as a
...
results can flatter a scheme through error cancellation
...
\section{Nonconforming overlap I: transfer fidelity}
\label{sec:fidelity}
...
looks small ($\pm 1\%$ at $1$\,ms), but the channel decomposition shows this
is a cancellation of two error channels, each moving roughly half the system
energy per millisecond:

\begin{center}
\small
\begin{tabular}{lccc}
\toprule
1\,ms, II, PU metric & conforming & nonconforming $h$ & nonconforming $h/2$ \\
\midrule
...
\subsection{The variational transfer and the fidelity ladder}
...
The measured ladder at resolution $h$ (dashpot work and net drift at
$1$\,ms):
...
\begin{tabular}{lcc}
\toprule
1\,ms, II, PU metric, $h$ & dashpot work & net PU drift \\
\midrule
...
Every fidelity increase makes the interface energy exchange larger. The
variational transfer does restore an $h$-dependence that pointwise
interpolation lacks (dashpot work $726 \to 484$\,J under refinement, versus
unchanged), but the error class does not change, and at production
resolution the net worsens because projecting the kinematics decorrelates
the kinematic-transfer error from the recovered-traction error, breaking the
...
pairing between traction and velocity drives the residual. The conclusion is
structural, not implementational: \emph{on nonmatching grids, content that
cannot be represented identically across the interface must be dissipated;
smoothing the transferred data is beneficial, and nodal stress recovery had
been acting as a load-bearing low-pass filter.}
...
baseline. Because the operator is verified, the null is informative:
...
$\beta = 0.3025$) collapses the dashpot work from $-607$ to $-231$\,J and
...
subdomain-level damping, not by the interface. ($\gamma = 0.6$ is a blunt
instrument --- it also removed $87\%$ of the physical energy on this
broadband benchmark.)
...
Mild HHT barely touches the interface channels even though
...
loss vs.\ $17\%$ at the same $\rho_\infty$). Damping converts the fragile
$\pm 1\%$ net (sign-indefinite, growth-capable) into monotone, bounded
decay at a cost proportional to how much of the problem's energy lives in
the reverberating band; on this benchmark that fraction is large by
construction (the parabolic release has no scale separation), whereas
production problems with smooth loading and resolution margin should obtain
the robustness at small cost with $\bar\alpha \approx 0.02$--$0.05$.

\section{Nonconforming overlap II: the parametric energy study}
\label{sec:study}

\subsection{Integrator--overlap--ratio sweep}
\label{sec:observed}

Drift $E(1\,\mathrm{ms})/E(0) - 1$ in percent, pointwise transfer, across
integrator pairs, overlap widths, and mesh ratios:
...
Three regimes: (a) conforming implicit--implicit runs conserve to
...
Integrated interface work over $1$\,ms (II runs, both subdomains):
...
absorbing boundary absorbs outgoing content. Third, and decisively for the
...
with Dirichlet overlap coupling (II), in both transfer flavors:
...
A finer-grained sweep (originating in a parametric study by our intern
Lukas, then confirmed and extended) coarsens the clamped mesh through
$r = 1.0, 0.9, \dots, 0.2$ at all three overlap widths and all four
integrator pairs, with variational transfer throughout --- $108$ runs. The
...
\begin{description}[style=unboxed,leftmargin=0pt,itemsep=2pt]
  ...
  \item[An injection resonance.] At $0.0254$/$1{:}0.3$ the coupling drives
  the beam to $+174\%$ (II) and $+275\%$ (IE) of $E(0)$, growing smoothly
  for ${\sim}0.7$\,ms and saturating near $3$--$4\times E(0)$ --- steady
  interface pumping, not a numerical divergence. At this ratio the clamped
  mesh has degenerated to a single-element cross-section, but degeneracy
  alone does not decide the outcome: the same clamped mesh at quarter
  overlap dissipates $-30.5\%$, and the explicit-clamped pairs on the
  identical configuration dissipate $-81\%$. Whether the sign-indefinite
  interface work injects or absorbs is a property of the mesh pair and the
  overlap geometry together.
...
be trusted with a drift budget: it cannot. The interface work is
...
\section{Theoretical requirements for energy consistency}
\label{sec:theory}
...
$+10\%$ in Sec.~\ref{sec:observed}), where the explicitly evaluated dashpot
at lagged time levels injects energy that central difference
...
\begin{description}[style=unboxed,leftmargin=0pt,itemsep=2pt]
  ...
  rides on near-cancellation of two large channels.
\end{description}

\section{Redesign within the overlap family}
\label{sec:redesign}

Four redesign phases were formulated from the theory; their outcomes:

\begin{description}[style=unboxed,leftmargin=0pt,itemsep=2pt]
  \item[Phase 1 --- variational transfer by default (implemented,
  reframed).] The full adjoint pairing of \eqref{eq:adjoint} presumes a
  \emph{shared} interface on which the two sides' works telescope. The
  overlap variant has none: data are exchanged on two distinct surfaces
  $\Gamma_1, \Gamma_2$, so no pairing of their works exists, and energy
  consistency can come only from agreement of the two solutions in the
  overlap --- exactly what dispersion mismatch precludes at fixed $h$
  (Sec.~\ref{sec:content-aware}). The implementable core of Phase~1 is the
  contraction requirement: \code{transfer: variational} is the default for
  the impedance overlap condition (pointwise remains an explicit legacy
  option; the two coincide on node-aligned interfaces). Empirically this
  converts the worst sign-indefinite case ($+554$\,J of positive dashpot
  work) into sign-correct controlled dissipation and improves $17$ of $24$
  nonconforming cases, but it cannot eliminate the dispersion-jump
  dissipation, and isolated growth cells persist.
  \item[Phase 2 --- dashpot on the representable jump (implemented;
  falsified).] Replace $~Z\jump{\dot{~u}}$ by
  $~Z\,\widehat\Pi\,\jump{\dot{~u}}$, where $\widehat\Pi$ is the
  $W$-orthogonal projection onto the transferable span (\code{representable
  dashpot: true}; a strict no-op on node-aligned interfaces). The premise
  was that the persistent jump is trace content the coarse space cannot
  represent, so that filtering it would let it reflect (conserving) instead
  of being absorbed. Measurement falsifies the premise: on the
  $0.0127$/3:4 case the fine side's $\widehat\Pi$ is a genuine projection
  ($\norm{\widehat\Pi - I}/\sqrt{n} = 0.48$) yet the dashpot work is
  unchanged ($-1066 \to -1065$\,J) --- the jump lies \emph{inside} the
  representable subspace. The persistent jump is a solution-content defect,
  not a spatial-representability defect: coarse-grid dispersion phase-lags
  the transiting pulse, so at Schwarz convergence the two discrete solutions
  disagree by a smooth, in-band field that no static spatial filter can
  remove. The option is retained (default off) as a diagnostic of where the
  jump lives.
  \item[Phases 3 and 4 --- harmonic-mean impedance; implicit interface for
  explicit subdomains.] Both belong structurally to the nonoverlap variant,
  where they were implemented as part of the adjoint pairing
  (Sec.~\ref{sec:paired}): the pair impedance $2Z_1 Z_2/(Z_1 + Z_2)$ per
  wave family (the Riemann-solver weighting of Duru et al.\ and Liu et
  al.), and the IMEX interface treatment (Sec.~\ref{sec:imex}).
\end{description}

The channel-resolved interface-work instrumentation remains the regression
metric: the dashpot work must be $\approx 0$ on conforming meshes at Schwarz
tolerance, and the study's drift tables are the acceptance benchmark for any
future change to the transmission machinery. A universal dashpot sign test
is \emph{not} attainable in the overlap variant, for the structural reason
...
is actually available. It has been implemented and measured on the
cantilever benchmark, split at $x = 0.127$\,m, with the mesh-ratio family of
...
and on the interface energy bookkeeping en route. What consistency does
...
\eqref{eq:telescope} guarantees. Second, near-conservation through mild
contrast ($-0.03\%$ at $1{:}0.9$, where every other variant tested loses or
gains several percent). Third, the residual dissipation at strong contrast
is the genuine trace-space jump being absorbed at rate
$Z\!\int_\Gamma \jump{\dot{~u}}^2$: at the exactly nested ratio $1{:}0.5$
(where the legacy transfer is coincidentally adjoint and the quadrature is
exact) paired and legacy agree to $0.1\%$, so the remaining loss is not a
transfer artifact but the representability price of
...
is the IMEX structure of Liu et al.: because the dashpot force is linear in
the unknown acceleration, the explicit update is closed implicitly on the
interface rows only,
\begin{equation}
...
$t_{n+1}$ exactly as in the implicit--implicit case. Drift at $1$\,ms, all
combinations, staggered metric \eqref{eq:staggered} for explicit members:
...
combinations conserve to $\le 0.3\%$; the nonconforming columns carry the
same monotone, sign-definite trace-jump dissipation in every combination.
(The small positive EI entries, $+0.2$--$0.3\%$, are bounded transient
storage in the conservative interface spring, not secular growth.) In the
consistent-mass integer-step metric the explicit columns instead read $-4$
to $-37\%$; the difference is the metric artifact of
Sec.~\ref{sec:benchmark}, not the coupling --- against the monodomain
explicit reference the conforming paired EE trajectory is exact to $0.35\%$
\emph{pointwise over the whole millisecond}: the coupled trajectory is the
monodomain trajectory, as the fixed-point argument predicts.
...
value for any query in the last segment and extrapolated backward
elsewhere). Second, the per-stop history was anchored at the first substep,
not the stop start, so a non-subcycling partner's history was a single
end-of-stop entry. Net effect: both directions of the exchange end-sampled
the partner's stop-end state --- constant, not interpolated, data. With
both defects fixed the exchange is genuinely piecewise-linear in both
directions. Drift at $1$\,ms, free side subcycled $4{:}1$
($\Delta t = 0.25\,\mu$s inside a $1\,\mu$s stop; consistent-mass
integer-step metric for comparability across the development history):
...
one percent on conforming meshes.) Genuine interpolation wins decisively at
mesh contrast, where end-sampling aliases the fine side's intra-window
...
discretization errors is not a conservation property, and it collapses
exactly where the stakes are high; the interpolated exchange is the
...
\begin{description}
  ...
  and passes the short regression --- and over hundreds of stops it injects
  energy catastrophically ($+10^3$--$10^5\%$ before aborting): a zero-lag
  filtered estimate is an extrapolation, and its frequency response exceeds
  unity off the linear band.
\end{description}
...
exchange; within the Schwarz family, genuinely interpolated
...
overlap runs, $74$ died of element inversion after exponential energy
growth (typically $+3{,}000$ to $+10{,}000\%$ of $E_0$ at death, between
$2$ and $9$\,ms), and most of the remainder collapsed toward zero energy.
The sign-indefinite drifts of the $1$\,ms sweep were early sections of
these trajectories: configurations that read as dissipative at $1$\,ms
(e.g.\ maximum overlap $1{:}0.6$ II, $-61\%$) reversed and grew to
$+5{,}432\%$ at death. Mesh conformity does not save the mixed-integrator
combinations: the conforming half-overlap IE run read $+13\%$ at $1$\,ms,
grew cleanly exponentially, and died at $6.25$\,ms; conforming EI behaves
alike at all three widths. The only healthy overlap configurations at
$10$\,ms are the conforming II runs: $+0.64$, $+0.61$, $+0.16\%$ at the
three overlap widths (Fig.~\ref{fig:overlap-10ms}). The overlap coupling is
therefore a short-horizon method on nonconforming meshes, and for non-II
combinations even on conforming ones.
...
run dead by element inversion at $t$\,ms; EE entries marked $\ast$ ended in
the noise-saturated state discussed below):
...
Three observations (Fig.~\ref{fig:nonoverlap-10ms}). First, the conforming
and $1{:}0.9$ columns with an implicit member are excellent --- the
coupling holds $0.00$ to $-0.9\%$ over ten thousand stops. Second, at
strong contrast ($\le 1{:}0.6$) the sign-definite dissipation that read as
$10$--$28\%$ at $1$\,ms reaches $40$--$85\%$ by $10$\,ms: bounded and
ordered, but far too large for wave-transit accuracy. Absorption of
unrepresentable content is a rate, not a one-time toll. Third, the mildly
nonconforming, \emph{non-nested} ratios $1{:}0.7$ and $1{:}0.8$ form a
failure window invisible at $1$\,ms: the paired transfer accumulates
interface-local high-frequency displacement content, and every integrator
combination eventually dies there --- explicit members at $2.6$--$4$\,ms by
central-difference instability, implicit pairs at $\approx 5$\,ms when the
first interface element inverts while the global energy is still smooth
($-19\%$ at death). This is not a relaxation artifact: reruns with fixed
$\theta = 0.5$ die identically (trajectories matching to three digits), so
the noise accumulation belongs to the transfer operators themselves. The
nested ratios ($1{:}0.5$ exactly, and the strong ratios whose transfer
filters aggressively) do not exhibit it.
...
conforming run holds $E/E_0 \ge 0.97$ to $5$\,ms, then slides), but the
...
dissipation at the stability edge, so whatever grid-frequency content the
...
Subcycling cuts the other way for II: the same-step conforming II run
conserves to $+0.00\%$ while the subcycled one loses $-20.2\%$ (at
$1{:}0.5$, $-61.6\%$) --- the Gravouil--Combescure interpolation
dissipation of Sec.~\ref{sec:gc}, harmless per stop, compounds over $10^4$
stops. At long horizons the multi-time-step exchange is thus a trade: it
...
dies at the same instant. The overlap exchange interpolates partner
...
when an unclamped $\theta$ excursion feeds the Newton solver a divergent
iterate; fixed $\theta = 0.5$ takes $47.5$ (clean); and a waveform-pooled
Aitken (one $\theta$ from residuals pooled across the window's slots) takes
$55.1$ --- clean on II but fatal on EE, where it locks $\theta$
persistently negative. Aitken acceleration is therefore at best useless and
at worst fatal inside windows, and Norma applies it only to single-slot
stops, where its ${\approx}3\times$ sweep reduction is proven
(\code{aitken\_applies}); windowed stops use the configured
\code{relaxation parameter}, with $\theta = 1$ recommended --- the
impedance dashpot supplies the contraction, so no under-relaxation is
needed.
...
and still dies at $9.8$\,ms). And windows are never a cost optimization for
the nonoverlap exchange: the number of substep solves per unit time is
...
the honest remedy remains CFL margin via subcycling
(Sec.~\ref{sec:tenms}).
...
\item The interface work of the overlap coupling is sign-indefinite in
  ...
  at a $10$\,ms horizon $74$ of $108$ nonconforming overlap runs diverge.
  The nonconforming overlap coupling cannot be trusted with an energy
...
\item \textbf{Overlap decompositions}: conforming meshes with
  ...
  other overlap configuration is a short-transient method. On conforming
  node-aligned interfaces the consistent traction makes the impedance
  overlap condition exact to the Dirichlet-coupling reference.
