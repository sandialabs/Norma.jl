...
% boldtensors declares a `cmbboard' symbol font from the bbold family
% (bbold10/bbold12/...), which is not in a base TeX Live install and cannot be
% auto-generated here.  TeX instantiates every declared symbol font when math
% starts, so the missing bbold12 aborts the build even though this note never
% uses the blackboard-bold tensor glyphs (the active-" syntax) --- only the
% bold-italic tensors (active-~), which come from cmmib and are always present.
% Redirect the unused cmbboard font to the AMS blackboard font (msb, loaded by
% amssymb) so the note builds on any base TeX Live without the bbold package.
...
\paragraph{Open items.}  (i) Non-conforming interfaces are now
understood rather than open: the small net error is a knife-edge
cancellation of two ${\sim}40\%$-of-$E_0$ error channels (interface
dashpot dissipation versus traction-channel injection), carried by
mesh-scale content that reverberates between the two grids because
their discrete dispersion disagrees.  A systematic campaign
(variational $L^2$ transfer, subdivided quadrature, consistent traction
across the interface via an offset facet surface, single-operator
characteristic transfer, and a content-aware absorbing term --- all
verified by patch tests) showed that every increase in transfer
fidelity pumps the churn, while a dissipative integrator kills the
reverberation loop; the pointwise + recovered-stress transmission with
the physical impedance remains the production default, ideally paired
with mild high-frequency integrator damping.  See the companion note
\emph{Variational Transfer for Impedance-Overlap Schwarz on
Non-Conforming Meshes} for the measurements.  (ii) Pairings with an
explicit subdomain lose
