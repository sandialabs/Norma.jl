...
partner traction is extracted as a \emph{consistent boundary flux}
(Wheeler; Hughes): for each interface node $i$,
\begin{equation}
  \bigl[ ~W\, ~t_j \bigr]_i
  \;=\; \int_{\Gamma_k} ~t_j\, \varphi_i \,\dd\Gamma
  \;=\; -\sum_{e \,\in\, \mathcal{E}_i^{\mathrm{ext}}}
     \bigl( ~f^{\mathrm{int}}_e + ~M_e\, \ddot{~u}_e \bigr)_i,
  \label{eq:flux-traction}
\end{equation}
...
coincide on $\Gamma_k$ and \eqref{eq:flux-traction} enters the
right-hand side directly, with no mass-matrix inversion and no
recovery solve.  Implemented by \texttt{build\_flux\_traction\_patch!}
(setup; rebuilt on BC swaps) and
\texttt{impedance\_flux\_partner\_traction} (\texttt{schwarz.jl}).
...
first-order-in-$h$ interface dissipation that the consistent-flux
...
\item \textbf{Partner right-hand side.}
      ...
      is the consistent-flux weak traction \eqref{eq:flux-traction} on
...
\[
  ...
  \;\longrightarrow\;
  \underbrace{0.08\%}_{+\,\text{consistent-flux traction}}
\]
...
the recovered traction with the consistent flux
\eqref{eq:flux-traction} eliminates it: energy loss
...
it.  With the consistent-flux traction the error term is gone and the
...
transfer injection remains (a mortar transfer with a flux-consistent
pairing, in the spirit of \eqref{eq:flux-traction}, is the candidate
...
\item On an \emph{overlap} boundary, Norma.jl provides two variants:
      ...
      as the variationally consistent boundary flux
      \eqref{eq:flux-traction} on node-aligned interfaces (consistently
...
\item The partner-traction term is what makes the impedance overlap
      ...
      recovery) $\to$ 0.08\% (consistent flux) over 1\,ms,
