...
rows carry the inertia residual, not $~\sigma ~n$).  Instead, $~t_j$ is
obtained by interpolating the partner's nodal-recovered stress with the
same shape-function data used for $\dot{~u}_j$ and $~u_j$, and
contracting with $\Omega_k$'s outward surface normals.  \emph{Consistent}
(L$^2$-projected, Cholesky-factorized) nodal recovery is force-enabled
for any model that carries this BC type; lumped recovery's $O(h)$
nodal-stress error feeds directly into the transmission condition and
was measured to dominate the residual energy error (13.6\% vs.\ 1.5\%
with consistent recovery; Section~\ref{sec:findings}).
...
\item \textbf{Partner right-hand side.}
      $~f_\Gamma \leftarrow ~f_\Gamma + ~W\,\bigl(~t_j
      + ~Z\, \dot{~u}_j + \alpha\, ~u_j\bigr)$, where $~t_j$,
      $\dot{~u}_j$, and $~u_j$ are obtained by strong interpolation from
      the partner element containing each overlap node
      (\texttt{apply\_bc\_detail} for
...
control (II, PU metric, physical impedance, $\alpha = 0$), the maximum
energy deviation over 1\,ms was
\[
...
\]
against the $0.1$\% exact-DBC reference.  Both corrections are
parameter-free.
...
is the theoretically correct operator for bimaterial interfaces.  On
this homogeneous, flexure-dominated benchmark it is slightly
\emph{detrimental} (conforming: 4.1\% vs.\ 1.5\% scalar; the tangential
channels of a flexural interface empirically prefer an impedance near
$Z_p$, not $Z_s$).  Its apparent advantage on the non-conforming pair
(3.9\% vs.\ 4.4\%) is another cancellation against transfer injection.
Transmission conditions should be benchmarked on conforming meshes;
non-conforming results flatter the scheme.

\paragraph{Open items.}  (i) The ${\sim}1.5$\% conforming residual
(candidates: Gauss--Seidel lag, the Newmark time level at which the
Robin terms are enforced).  (ii) The ${\sim}4$\% non-conforming
transfer injection (a variationally consistent mortar transfer is the
candidate fix).  (iii) Pairings with an explicit subdomain lose
${\sim}8$--$10$\% of the PU energy within the first few steps and
oscillate per step thereafter (${\sim}20$\% overall), before and after
the corrections alike: the discrete treatment of the impedance terms
under central difference is the suspect, consistent with the
literature's warnings about discretizing absorbing boundary operators
explicitly.
...
\item On an \emph{overlap} boundary, Norma.jl provides two variants:
      ...
      from consistently recovered nodal stress and the tensor impedance
      $~Z$ built from the partner's P- and S-wave impedances.
\item The partner-traction term and consistent stress recovery are what
      make the impedance overlap condition consistent with the
      monodomain solution: on the conforming cantilever benchmark they
      reduce the maximum PU-energy deviation from 59\% to 1.5\% over
      1\,ms, parameter-free (physical impedance, $\alpha = 0$), against
      a 0.1\% exact-conservation reference for converged conforming DBC
      coupling.  Converged DBC coupling on \emph{non-conforming} meshes
      is exponentially unstable (transfer-induced); the impedance
      variant's velocity-jump dissipation is what keeps it bounded
      there.
\end{itemize}
