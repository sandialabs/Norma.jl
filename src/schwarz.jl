...
# Bochev-Kuberry interface mass correction for non-overlap DN Schwarz.
# In a monolithic solve, interface nodes have mass from both sides' volume
# elements. When partitioned, the NBC side only sees its own mass — roughly
# half the monolithic value. This causes wrong effective impedance and energy
# drift, especially with explicit integrators where ü = F/m directly.
#
# The correction adds the partner's projected volume-mass to the NBC side's
# interface DOFs, restoring the monolithic mass at the interface.
# Ref: Bochev & Kuberry, PANACM 2015, Eqs. 24–27.
function apply_interface_mass_correction!(sim::MultiDomainSimulation)
    for subsim in sim.subsims
        ...
        for bc in model.boundary_conditions
            ...
            partner_model = bc.coupled_subsim.model
            partner_bc = partner_model.boundary_conditions[bc.coupled_bc_index]

            w_self = vec(sum(bc.square_projector, dims=2))
            w_partner = vec(sum(partner_bc.square_projector, dims=2))

            m_partner = _interface_lumped_mass(partner_model, partner_bc)
            μ_partner = m_partner ./ w_partner
            μ_self = bc.dirichlet_projector * μ_partner
            Δm = w_self .* μ_self

            if !isempty(model.lumped_mass)
                for (i_local, i_global) in enumerate(bc.global_from_local_map)
                    for comp in 1:3
                        dof = 3 * (i_global - 1) + comp
                        model.lumped_mass[dof] += Δm[i_local]
                    end
                end
            end
...
function _interface_lumped_mass(model::SolidMechanics, bc::SolidMechanicsSchwarzBoundaryCondition)
    n = length(bc.global_from_local_map)
    m = zeros(n)
    for (i_local, i_global) in enumerate(bc.global_from_local_map)
        dof = 3 * (i_global - 1) + 1
        if !isempty(model.lumped_mass)
            m[i_local] = model.lumped_mass[dof]
        elseif size(model.mass, 1) > 0
            m[i_local] = model.mass[dof, dof]
        end
    end
    return m
end
