...
function should_swap(c::TimeSwapCriterion, sim::Simulation)
    # Use prev_time (the last successfully completed step) rather than time
    # (the next target time, already advanced by advance_control_time).
    # Using `time` causes two problems:
    #   1. The swap fires and is logged one step early — at the *next* target
    #      time rather than at the step that crossed the threshold.
    #   2. When a replacement YAML's swap plan has a t_swap equal to the
    #      current target time (e.g. t_swap=0.6 and time=6*0.1≈0.6+ε), the
    #      newly-appended plan fires immediately in the same maybe_apply_swaps!
    #      loop iteration due to floating-point accumulation.
    # Using prev_time >= t_swap fires the swap as soon as the last completed
    # step reaches the threshold, and a freshly-appended plan whose t_swap is
    # ahead of prev_time will not fire until the correct step.
    return sim.controller.prev_time >= c.t_swap
end
...
function maybe_apply_swaps!(sim::SingleDomainSimulation)
    ...
    for plan in sim.swaps
        ...
        # apply_single_domain_swap! mutates sim in-place (model, controller,
        # integrator, solver, and potentially sim.swaps via the append logic).
        # Stop after one swap per call: the next maybe_apply_swaps! invocation
        # (on the following evolve step) will pick up any further pending plans
        # with updated controller state, avoiding double-fires within one step.
        break
    end
    return nothing
end
...
function apply_single_domain_swap!(sim::SingleDomainSimulation, plan::SwapPlan)
    ...
    sim.model = new.model
    # Do NOT replace sim.swaps with new.swaps wholesale.  The plans already in
    # sim.swaps carry their applied flags (the current plan is about to be
    # marked applied below), so discarding them resets those flags and causes
    # every time-based criterion whose t_swap is already in the past to fire
    # again on the very next evolve iteration — producing runaway re-swapping.
    # Instead, append only the swap plans that are genuinely new (i.e. not
    # already present in sim.swaps by replacement file), so that a replacement
    # YAML can introduce additional downstream swaps without clobbering the
    # history of what has already been applied.
    existing_replacements = Set(p.replacement_file for p in sim.swaps)
    for p in new.swaps
        p.replacement_file ∈ existing_replacements || push!(sim.swaps, p)
    end
