...
function initialize_writing(sim::SingleDomainSimulation)
    ...
    if !(sim.model isa RomModel) && !(sim.model.recovery_data isa NoRecovery)
        num_node_vars += 6
        if (sim.model.recovery_data isa ConsistentRecovery)
        ...
        elseif (sim.model.recovery_data isa LumpedRecovery) 
...
function write_stop_exodus(sim::SingleDomainSimulation, model::SolidMechanics)
    ...
    if !(model.recovery_data isa NoRecovery)
        ...
        if (model.recovery_data isa ConsistentRecovery)
            Exodus.write_values(output_mesh, NodalVariable, time_index, "consistent_sigma_xx_n", nodal_sigma[1, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "consistent_sigma_yy_n", nodal_sigma[2, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "consistent_sigma_zz_n", nodal_sigma[3, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "consistent_sigma_yz_n", nodal_sigma[4, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "consistent_sigma_xz_n", nodal_sigma[5, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "consistent_sigma_xy_n", nodal_sigma[6, :])
        elseif (model.recovery_data isa LumpedRecovery)
            Exodus.write_values(output_mesh, NodalVariable, time_index, "lumped_sigma_xx_n", nodal_sigma[1, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "lumped_sigma_yy_n", nodal_sigma[2, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "lumped_sigma_zz_n", nodal_sigma[3, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "lumped_sigma_yz_n", nodal_sigma[4, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "lumped_sigma_xz_n", nodal_sigma[5, :])
            Exodus.write_values(output_mesh, NodalVariable, time_index, "lumped_sigma_xy_n", nodal_sigma[6, :])
        end 
